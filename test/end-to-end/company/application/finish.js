...
describe("POST /company/application/finish", () => {
    ...
    describe("With Auth", () => {
        ...
        beforeAll(async () => {
            ...
            const testFinishedCompany = await Company.create(finishedCompanyData);
            await Account.create({
        ...
        afterAll(async () => {
            await Company.deleteMany({ name: finishedCompanyData.name });
            await Account.deleteMany({ isAdmin: true });
            await Account.deleteMany({ email: testFinishedUser.email });
        });
        ...
        describe("Contacts", () => {
            ...
            afterAll(async () => {
                await Company.deleteMany({ name: nonFinishedCompanyData.name });
                await Company.deleteMany({ name: nonFinishedSingleContactCompanyData.name });
                await Account.deleteMany({ email: testUser.email });
                await Account.deleteMany({ email: testSingleContactUser.email });
            });
